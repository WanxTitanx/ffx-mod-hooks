...
namespace SinCoreLib.Encoding
{
    public partial class FfxEncoding
    {
        ...
        public static string DecodeString(TextScript.TextCommand stringCommand, Dictionary<byte, char> decoder)
        {
            ...
            string finalString = "";
            byte[] bytes = stringCommand.ByteArray;
            for (int i = 0; i < bytes.Length; i++)
            {
                byte b = bytes[i];

                // 2-byte glyph (alternate font bank): lead + next -> a readable token (e.g. <FTCX:5>).
                // The kanji banks (0x06, 0x26..0x2F) used to be mis-read as 1-byte <MISS>; now they decode.
                if (IsGlyphLead(b) && i + 1 < bytes.Length
                    && TryDecodeGlyph(b, bytes[i + 1], out string glyphToken))
                {
                    finalString += glyphToken;
                    i++; // consume the trailing byte of the pair
                    continue;
                }
        ...
        {
            // A null script (e.g. monmagic's empty JP-only text fields, or a field that failed to
            // round-trip through a wrapper) is an empty script — coalesce so we never NPE here.
            bytesToWrite ??= System.Array.Empty<byte>();
            // Add NULL byte (0)
