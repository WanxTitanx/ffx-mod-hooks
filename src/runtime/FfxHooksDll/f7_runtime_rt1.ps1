# F7 Difficulty isolated concurrent runtime harness (x86, no game process).
...
$objDir = Join-Path $PSScriptRoot 'obj\rt1'
New-Item -ItemType Directory -Path $objDir -Force | Out-Null
$sources = @(
    (Join-Path $PSScriptRoot 'tests\F7RuntimeRt1.cpp'),
    (Join-Path $PSScriptRoot 'hooks\F7DifficultyCore.cpp'),
    (Join-Path $PSScriptRoot 'hooks\SinRamScalingCore.cpp'),
    (Join-Path $PSScriptRoot 'hooks\SinRamConfigCore.cpp'),
    (Join-Path $PSScriptRoot 'hooks\F7ConfigState.cpp'),
    (Join-Path $PSScriptRoot 'hooks\MinHookBatchCoordinator.cpp')
)
$sourceArguments = ($sources | ForEach-Object { '"{0}"' -f $_ }) -join ' '
$exe = Join-Path $objDir 'F7RuntimeRt1.exe'
$pdb = Join-Path $objDir 'F7RuntimeRt1.pdb'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "F7 runtime RT1 compilation failed with exit code $LASTEXITCODE" }
    & $exe
    if ($LASTEXITCODE -ne 0) { throw "F7 runtime RT1 executable failed with exit code $LASTEXITCODE" }
} finally {
    Pop-Location
}

Write-Host 'F7 RUNTIME RT1: PASS'
