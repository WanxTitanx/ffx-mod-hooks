...
$objDir = Join-Path $PSScriptRoot 'obj\ronso-pool-rt0'
New-Item -ItemType Directory -Path $objDir -Force | Out-Null
$sources = @(
    (Join-Path $PSScriptRoot 'tests\RonsoPoolRt0.cpp'),
    (Join-Path $PSScriptRoot 'hooks\RonsoPoolCore.cpp')
)
$sourceArguments = ($sources | ForEach-Object { '"{0}"' -f $_ }) -join ' '
$exe = Join-Path $objDir 'RonsoPoolRt0.exe'
$pdb = Join-Path $objDir 'RonsoPoolRt0.pdb'
...
Write-Host 'RONSO POOL RT0: PASS'
