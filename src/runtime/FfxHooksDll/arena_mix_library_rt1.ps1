param([Parameter(Mandatory=$true)][string]$CarrierPath,[Parameter(Mandatory=$true)][string]$ProfilesPath,[string]$EvidenceDirectory,[string]$LegacyProfilesPath)
...
$objDir = Join-Path $PSScriptRoot 'obj\arena-mix-library-rt1'
New-Item -ItemType Directory -Path $objDir -Force | Out-Null
$sources = @(
    (Join-Path $PSScriptRoot 'tests\ArenaMixLibraryRt1.cpp'),
    (Join-Path $PSScriptRoot 'hooks\ArenaMixLibrary.cpp'),
...
$exe = Join-Path $objDir 'ArenaMixLibraryRt1.exe'
$pdb = Join-Path $objDir 'ArenaMixLibraryRt1.pdb'
...
try {
    ...
    & $exe $CarrierPath $ProfilesPath $EvidenceDirectory $LegacyProfilesPath
    if ($LASTEXITCODE -ne 0) { throw "CustomMix runtime executable failed with exit code $LASTEXITCODE" }
} finally {
    Pop-Location
}

Write-Host ('Arena Mix library RT1: PASS (RT0 contracts + RT1 composition; COFF-I386 0x{0:X4})' -f $machine)
