...
$obj = Join-Path $here 'obj\ronso-pool-native-rt1'
...
$exe = Join-Path $obj 'RonsoPoolNativeRt1.exe'
...
$compile = 'call "{0}" x86 >nul && cl /nologo /EHsc /std:c++17 /W4 /WX /utf-8 /MT /DFFXHOOKS_HAVE_POLYHOOK /DFFXHOOKS_TESTING /I"{1}\include" "{2}\tests\RonsoPoolNativeRt1.cpp" "{2}\hooks\MinHookBatchCoordinator.cpp" "{2}\hooks\F8RuntimeCore.cpp" "{2}\hooks\RonsoPoolCore.cpp" "{2}\hooks\RonsoPoolSave.cpp" "{2}\hooks\RonsoPoolStore.cpp" "{2}\hooks\RonsoPoolRuntime.cpp" hook.obj buffer.obj trampoline.obj hde32.obj /Fe"{3}" bcrypt.lib' -f $vcvars,$mh,$here,$exe
Push-Location $obj
try {
    ...
    & $exe $fixture $save $root
    if ($LASTEXITCODE -ne 0) { throw 'Nova RT1 failed' }
} finally { Pop-Location }
Write-Host 'RONSO POOL NATIVE RT1: PASS'
