# F7 Difficulty runtime RT0 harness (x86, isolated from the DLL project).
...
$sources = @(
    (Join-Path $PSScriptRoot 'tests\F7RuntimeRt0.cpp'),
...
$exe = Join-Path $objDir 'F7RuntimeRt0.exe'
$pdb = Join-Path $objDir 'F7RuntimeRt0.pdb'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "F7 runtime RT0 compilation failed with exit code $LASTEXITCODE" }
    & $exe
    if ($LASTEXITCODE -ne 0) { throw "F7 runtime RT0 executable failed with exit code $LASTEXITCODE" }
} finally {
    Pop-Location
}

Write-Host 'F7 RUNTIME RT0: PASS'
