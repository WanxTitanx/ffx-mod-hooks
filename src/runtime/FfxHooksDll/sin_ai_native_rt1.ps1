param([Parameter(Mandatory=$true)][string]$ExecutablePath,[Parameter(Mandatory=$true)][string]$PackPath,[Parameter(Mandatory=$true)][string]$CommandsPath,[Parameter(Mandatory=$true)][string]$MonstersPath)
...
$obj=Join-Path $PSScriptRoot 'obj\sin-ai-native-rt1'
...
$sources=@('tests\SinAiNativeRt1.cpp','hooks\SinAiHook.cpp','hooks\F8RuntimeCore.cpp') | ForEach-Object {'"'+(Join-Path $PSScriptRoot $_)+'"'}
$exe=Join-Path $obj 'SinAiNativeRt1.exe'
Push-Location $obj
try {
    ...
    & $exe $ExecutablePath $PackPath $CommandsPath $MonstersPath
